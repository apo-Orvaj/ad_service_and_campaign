  ...
  def self.compare
	...
	ads.each_with_index do |ad, index|
		...
		unless ad["status"] == campaign_obj.status
			discrepancies << {"status": {"remote": ad["status"], "local": campaign_obj.status}}
		end
		...
		result << { "remote_reference": campaign_obj.external_reference, "discrepancies": discrepancies }
	end
	result
  end
end
